...
class FlightLog:
    ...
    """
    def __init__(self, vehicle, logtime=.5, position=True, attitude=True, gyro=True, accel=True, 
            barom=True, heading=True, battery=True, sys=False, start_time=None):
        """
        ...
        self._position_meters = position
    ...
    def _header(self):
        ...
        if self._position_meters:
            line += ",North_m,East_m,Down_m"
        if self._attitude:
            line += ",Pitch,Roll,Yaw"
        if self._gyro:
    ...
    def _logline(self):
        ...
        if self._position:
            pos = "," + str(self._vehicle.location.global_frame.lat) #north
            pos += "," + str(self._vehicle.location.global_frame.lon) #east
            pos += "," + str(self._vehicle.location.global_frame.alt) #down
            line += pos
        ...
        if self._heading:
            if self._heading: #heading has been updated
                line += "," + str(self._vehicle.heading) #press_abs
            else: #heading has not been updated,do not re-record
                line += ","
